...
const { createNotification, getOrCreateReminderSettings } = require('../services/notificationService');
...
async function updateChecklist(req, res, next) {
  try {
    ...
    const beforeEntry = checklist[date] || {};
    const dayEntry = { ...beforeEntry };
    if (done) {
    ...
    // Real completion notification: fired only on the exact transition
    // from "not all steps done" to "all steps done" for morning or
    // evening on this date, so it's never sent more than once per
    // half-routine per day and never claims a completion that didn't
    // just happen.
    try {
      if (done && item_key.startsWith('morning-') || (done && item_key.startsWith('evening-'))) {
        const half = item_key.startsWith('morning-') ? 'morning' : 'evening';
        const steps = half === 'morning' ? plan.morning_routine : plan.evening_routine;
        const keysForHalf = (steps || []).map((s, i) => `${half}-${s.step ?? i}`);
        const wasComplete = keysForHalf.length > 0 && keysForHalf.every((k) => beforeEntry[k]);
        const isComplete = keysForHalf.length > 0 && keysForHalf.every((k) => dayEntry[k]);

        if (!wasComplete && isComplete) {
          const settings = await getOrCreateReminderSettings(req.user.id);
          if (settings.progress_alerts_enabled) {
            const label = half === 'morning' ? '🌞 Morning routine complete' : '🌙 Evening routine complete';
            await createNotification({
              userId: req.user.id,
              type: 'PROGRESS_ALERT',
              title: label,
              message: `Nice work — you completed all ${keysForHalf.length} step(s) of your ${half} routine today.`,
            });
          }
        }
      }
    } catch (notifyErr) {
      console.error('Routine completion notification failed (non-fatal):', notifyErr.message);
    }

    res.json({ message: 'Checklist updated.', plan });
  } catch (err) {
    next(err);
  }
}
