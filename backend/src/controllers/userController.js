...
async function uploadAndAnalyze(req, res, next) {
  try {
    ...
    // Pull the previous report BEFORE inserting the new one, so we can
    // raise a real, numbers-backed progress alert afterward.
    const { rows: prevRows } = await pool.query(
      'SELECT skin_health_score, created_at FROM skin_reports WHERE user_id = $1 ORDER BY created_at DESC LIMIT 1',
      [req.user.id]
    );
    ...
    // Real progress-alert notification: only sent when we have an actual
    // previous score to compare against, and the message states the
    // exact delta — never a vague "your skin improved" claim.
    try {
      const settings = await getOrCreateReminderSettings(req.user.id);
      if (settings.progress_alerts_enabled && previous && Number.isFinite(Number(previous.skin_health_score))) {
        const delta = Math.round(Number(analysis.skin_health_score) - Number(previous.skin_health_score));
        const direction = delta > 0 ? 'improved by' : delta < 0 ? 'decreased by' : 'stayed the same at';
        const magnitude = delta !== 0 ? `${Math.abs(delta)} pts` : '';
        await createNotification({
          userId: req.user.id,
          type: 'PROGRESS_ALERT',
          title: '📊 Progress update',
          message: delta !== 0
            ? `Your skin health score ${direction} ${magnitude} since your last assessment (${previous.skin_health_score} → ${analysis.skin_health_score}).`
            : `Your skin health score ${direction} ${analysis.skin_health_score} since your last assessment.`,
        });
      }
    } catch (notifyErr) {
      console.error('Progress alert notification failed (non-fatal):', notifyErr.message);
    }
