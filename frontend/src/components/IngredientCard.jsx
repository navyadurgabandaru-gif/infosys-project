 ...
 * plain catalog browse. `onLearnMore` (optional) renders a button that
 * opens the full Ingredient Education view (description, usage,
 * precautions, interaction warnings) for this ingredient.
 */
export default function IngredientCard({ ingredient, reason, verdict, onLearnMore }) {
  ...
  return (
    <div className="ingredient-card">
      ...
      {comedogenic_rating != null && (
        <div className="card-mini-label" style={{ marginTop: 6 }}>
          Comedogenic rating: <span style={{ color: 'var(--color-text)', fontWeight: 700 }}>{comedogenic_rating}/5</span>
        </div>
      )}
      ...
      {onLearnMore && (
        <button type="button" className="btn btn-outline btn-sm" style={{ marginTop: 10, alignSelf: 'flex-start' }} onClick={() => onLearnMore(ingredient)}>
          📖 Learn More
        </button>
      )}
    </div>
