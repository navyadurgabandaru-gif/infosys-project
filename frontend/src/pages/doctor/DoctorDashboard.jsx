...
import PatientReportExportButtons from '../../components/PatientReportExportButtons';
    ...
    <div className="card">
      <div className="table-wrap">
        <table className="data-table">
          <thead><tr><th>Name</th><th>Email</th><th>Phone</th><th>Skin Type</th><th>Export</th></tr></thead>
          <tbody>
            {patients.map((p) => (
              <tr key={p.id}>
                <td>{p.name}</td><td>{p.email}</td><td>{p.phone || '—'}</td><td>{p.skin_type || '—'}</td>
                <td><PatientReportExportButtons userId={p.id} label={p.name} /></td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="modal-backdrop" onClick={() => setSelected(null)}>
          <div className="modal-box" style={{ maxWidth: 560 }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <PatientReportExportButtons userId={selected.user_id} label={selected.patient_name} />
            </div>
          </div>
        </div>
      )}
