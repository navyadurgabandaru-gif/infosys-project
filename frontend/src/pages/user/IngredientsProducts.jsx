...
import IngredientEducationModal from './IngredientEducationModal';
...
/**
 * Interaction Warnings: real conflicts among the ingredients found in the
 * user's CURRENTLY recommended products (GET /ingredients/for-me/interactions
 * — resolved server-side from their latest plan's product recommendations,
 * not a generic catalog list). Renders nothing misleading when there's no
 * routine yet or no conflicts exist — an honest empty state either way.
 */
function InteractionWarningsCard() {
  const [data, setData] = useState(undefined); // undefined = loading, null = error
  const [error, setError] = useState('');

  useEffect(() => {
    api.get('/ingredients/for-me/interactions')
      .then(({ data }) => setData(data))
      .catch((err) => {
        setData(null);
        setError(err.response?.data?.message || 'Could not check for ingredient interactions right now.');
      });
  }, []);
  ...
  if (data === undefined) {
    return (
      <div className="card plan-section">
        <h3 style={{ marginTop: 0 }}>⚠️ Interaction Warnings</h3>
        <Loading />
      </div>
    );
  }
  ...
  if ((data.routine_ingredient_names || []).length === 0) {
    return (
      <div className="card plan-section">
        <h3 style={{ marginTop: 0 }}>⚠️ Interaction Warnings</h3>
        <Empty label="No current routine to check yet — generate a skincare plan to see interaction warnings for your recommended products." />
      </div>
    );
  }
    ...
    return (
      <div className="card plan-section">
        ...
        <p className="text-muted" style={{ fontSize: 13.5 }}>
          No known conflicts found among the ingredients in your current recommended products. ✓
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="card plan-section">
      ...
      <p className="text-muted" style={{ fontSize: 13.5, marginTop: -6 }}>
        Found in the ingredients across your currently recommended products.
      </p>
      {interactions.map((w, idx) => (
        <div key={idx} className="card-warning" style={{ marginBottom: 8 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
            <strong>{w.ingredient_a.name} + {w.ingredient_b.name}</strong>
 ...
 */
function PersonalizedIngredientsSection({ onLearnMore }) {
    ...
    <>
      ...
      <div className="card plan-section">
        ...
        {suitable.length > 0 ? (
          <div className="card-grid">
            {suitable.map((i, idx) => <IngredientCard key={i?.id ?? idx} ingredient={i} reason={i?.reason} verdict="suitable" onLearnMore={onLearnMore} />)}
          </div>
        ) : (
      ...
      {avoid.length > 0 && (
        <div className="card plan-section">
          ...
          <div className="card-grid">
            {avoid.map((i, idx) => <IngredientCard key={i?.id ?? idx} ingredient={i} reason={i?.reason} verdict="avoid" onLearnMore={onLearnMore} />)}
          </div>
        </div>
      )}
      ...
      {caution.length > 0 && (
        <div className="card plan-section">
          ...
          <div className="card-grid">
            {caution.map((i, idx) => <IngredientCard key={i?.id ?? idx} ingredient={i} reason={i?.reason} verdict="caution" onLearnMore={onLearnMore} />)}
          </div>
        </div>
      )}
    </>
...
function ForYouTab({ onLearnMore }) {
  return (
    <div>
      <ProductRecommendationsSection />
      <InteractionWarningsCard />
      <PersonalizedIngredientsSection onLearnMore={onLearnMore} />
    </div>
  );
}

function BrowseTab({ onLearnMore }) {
  ...
  return (
    <div>
      ...
      {ingredients === undefined ? (
        ...
        <div className="card-grid">
          {ingredients.map((i, idx) => <IngredientCard key={i?.id ?? idx} ingredient={i} onLearnMore={onLearnMore} />)}
        </div>
      )}
    </div>
  );
...
export default function IngredientsProducts() {
  ...
  return (
    <div>
      ...
      {tab === 'for-you' && <ForYouTab onLearnMore={(ing) => setLearnMoreId(ing.id)} />}
      {tab === 'browse-products' && <BrowseProductsTab />}
      {tab === 'browse' && <BrowseTab onLearnMore={(ing) => setLearnMoreId(ing.id)} />}

      <IngredientEducationModal ingredientId={learnMoreId} onClose={() => setLearnMoreId(null)} />
    </div>
  );
}
